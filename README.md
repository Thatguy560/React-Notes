...
It should also be noted that the React team is supporting more React hooks for functional components that replace or even improved upon class components. The React ream mentioned in earlier days that they will make performance optimizations in functional components by avoiding unnecessary checks and memory allocations. As as promising as it sounds, new hooks are recently introduced for functional components such as **useState** or **useEffect** while also promising that are not going to obsolete class components. 
...
Here, **useState** is a Hook. We call it inside a function component to add some local state to it. React will preserve this state between re-renders. **useState** returns a pair: the current state value and a function that lets you update it. (State and Setter). You can call this function from an event handler or somewhere else. It's similar to this.setState in a class, except it doesn't merge the old and new state together. (We'll show an example comparing **useState** to this.state in the [Using the state hook](https://reactjs.org/docs/hooks-state.html)).

The only argument to **useState** is the initial state. In the example above the initial state is 0. Because our counter starts from zero. Unoike **this.state**, the state here doesn't have to be an object. Although it can be if you want. The initial state argument is only used during the first render.
...
You've likely performed data fetching, subscriptions, or manually changed the DOM from React components before. We call these operations "side effects" (or "effects" for short) because they can affect other components and can't be done during rendering.

The Effect Hook, **useEffect**, adds the ability to perform side effects from a function component. It serves the same purpose as **componentDidMount, componentDidUpdate**, and **componentWillUnmount** in React classes, but unified into a single API. Below is examples comparing **useEffect** to these methods in **Using the Effect Hook**.
...
When you call **useEffect**, you're telling React to run your "effect" function after flushing changes to the DOM. Effects are declared inside the component so they have access to props and state. By default, React runs the effects after every render - Including the first render. 

Effects may also optionally specify how to "clean up" after them by returning a function. For example, this component uses an effect to subscribe to a friends online status, and cleans up by unsubscribing from it:
...
function FriendStatus(props) {
    ...
    useEffect(() => {
        ...
        return () => { // Another way of writing ComponentDidUnmount
            chatAPI.unsubscribeToFriendsStatus(props.friends.id, handleStatusChange)
        };
    });
    if (isOnline === null) {
        return "Loading..."
...
Sometimes, we want to re-use some stateful logic between components. Traditionally, there were two popular solutions to this problem: **[higher-order components](https://reactjs.org/docs/higher-order-components.html)** and **[render props](https://reactjs.org/docs/render-props.html).** Custom Hooks let you do this, but without adding more components to your tree.

Earlier on this page, we introduced a **FriendStatus** component that calls the **useState** and **useEffect** Hooks to subscribe to a friend's online status. Let's say we also want to re-use this subscription logic in another component. 
...
function useFriendStatus(friendID) {
    ...
    useEffect(() => {
        ...
        return () => {
            chatAPI.unsubscribeToFriendsStatus(friendID, handleStatusChange);
        }
    })
    return isOnline;
}
...
The state of each component is completely independent. Hooks are a way to re-use stateful logic, not state itself. In fact, each call to a Hook has a completely isolated state - so you can even use the same custom Hook twice in one component.

Custom Hooks are more of a convention than a feature. If a function's name starts with **"use"** and it calls other Hooks, we can say it's a custom Hook. The **useSomething** naming convention is how our linter plugin is able to find bugs in the code using Hooks. 
...
There are few less commonly used built-in Hooks that you might find useful. For example, **useContext** lets you subscribe to React context without introducing nesting:
